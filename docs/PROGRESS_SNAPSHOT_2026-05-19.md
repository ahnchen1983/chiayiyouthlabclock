...
| **6** | 排班協作 | 🟡 進行中 | **1/4** | 6.3 ✅ ／ 6.1 / 6.2 / 6.4 待 |
| **7** | 系統健全化（技術債） | 🟡 進行中 | **5/7** | 7.1 / 7.3 / 7.4 / 7.5 / 7.7 ✅ ／ 7.2 / 7.6 待 |
| **8** | HR 細節補強 | 🟡 進行中 | **2/5** | 8.2 / 8.3 ✅ ／ 8.1 / 8.4 / 8.5 待 |
| **9** | 資安強化 | ✅ 完成 | **4/4** | 9.1 / 9.2 / 9.3 / 9.4 全綠 |

**已完成：** Phase 1–5 + Phase 9 + 部分 6/7/8（共 **30 個子項**）
**剩餘規劃中：** 8 個子項（Phase 6×3 / Phase 7×2 / Phase 8×3）
...
| 編號 | 名稱 | 工單檔 | 預估工期 |
|------|------|--------|---------|
| 6.1 | 換班 / 替班申請 | `PHASE_6.1_SHIFT_SWAP.md` | 1~2 天 |
| 6.2 | 排班版本歷史（snapshot） | `PHASE_6.2_SCHEDULE_VERSION.md` | 半天~1 天 |
| 6.4 | 員工偏好班次設定 | 工單尚未撰寫 | — |
...
| 編號 | 名稱 | 工單檔 | 預估工期 |
|------|------|--------|---------|
| 8.1 | 特休跨年結轉 | `PHASE_8.1_LEAVE_CARRYOVER.md` | 半天 |
| 8.4 | 月結報表（管理員月度統計） | `PHASE_8.4_MONTHLY_REPORT.md` | 1 天 |
| 8.5 | 員工自助申請流程（特休 / 留停） | 工單尚未撰寫 | — |
...
- **8.1 特休跨年結轉**：工單已備齊，純後端，半天可結

### 中線（1~2 天）

- **8.4 月結報表**：管理員價值高，工單已備齊
- **6.2 排班版本歷史**：搭配 6.3 月結鎖定的下游應用，工單已備齊

### 長線（需先寫工單）

- 7.2 Playwright e2e
- 7.6 FCM Push 通知
- 6.1 換班 / 替班申請（工單已備齊但 1~2 天大票）
- 6.4 員工偏好 / 8.5 員工自助申請
...
| 維度 | 狀態 | 備註 |
|------|------|------|
| 功能完整度 | 🟢 良好 | 客戶第一輪 + 第二輪需求全處理 + v2.0 重構 |
| 測試覆蓋 | 🟢 良好 | 133 個單元測試，純函數覆蓋紮實 |
