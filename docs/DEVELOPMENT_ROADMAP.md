...
| **Phase 5 排班模型重構（v2.0 核心）** | 🟡 進行中（資料模型已切換） | 5/7 |
...
| 5.2 | 應到人數欄位 + 時段覆蓋檢核（**警示不阻擋**） | V2-1b | 🟡 部分（requiredHeadcount + 衝突警示已就緒；30 分鐘區段覆蓋檢核留 5.3） |
| 5.3 | 排班 UI 重做：時間軸視覺化 + 每人時段編輯（含「+ 第二段」按鈕） | V2-2 / V2-3 | 🟡 部分（班次列表 UI 已可用，時間軸視覺化待後續） |
