...
| **Phase 5 排班模型重構（v2.0 核心）** | ⬜ 規劃中（決策已敲定） | 0/7 |
| **Phase 6 排班協作** | ⬜ 規劃中 | 0/4 |
| **Phase 7 系統健全化（技術債）** | ⬜ 規劃中 | 0/7 |
| **Phase 8 HR 細節補強** | ⬜ 規劃中 | 0/5 |
...
| 編號 | 項目 | 對應問題 | 狀態 |
|------|------|---------|------|
| 5.1 | ScheduleEvent 改為 `shifts: StaffShift[]`（兩頭班 ≤ 2 段） | V2-2 / V2-3 / V2-5 | [ ] |
| 5.2 | 應到人數欄位 + 時段覆蓋檢核（**警示不阻擋**） | V2-1b | [ ] |
| 5.3 | 排班 UI 重做：時間軸視覺化 + 每人時段編輯（含「+ 第二段」按鈕） | V2-2 / V2-3 | [ ] |
| 5.4 | 修 bug — 休館(值班) 兼職在日曆顯示（**允許排**） | V2-4 | [ ] |
| 5.5 | Onboarding wizard — **清空 dailySchedule + scheduleTemplate** 工具 | V2-1a / 決策 4 | [ ] |
| 5.6 | 薪資計算 / 遲到判定 / 打卡比對遷移到新模型 | 5.1 連動 | [ ] |
...
| 編號 | 項目 | 狀態 |
|------|------|------|
| 6.1 | 換班/替班申請（員工互換 + 對方確認） | [ ] |
| 6.2 | 排班版本歷史（snapshot + 回溯） | [ ] |
| 6.3 | 月結鎖定：薪資結算後該月排班/打卡不可改 | [ ] |
| 6.4 | 員工偏好班次設定（可上/不可上時段） | [ ] |
...
| 編號 | 項目 | 狀態 |
|------|------|------|
| 7.1 | Vitest 單元測試（薪資、遲到、餘額計算） | [ ] |
| 7.2 | Playwright e2e（登入→打卡→請假→審核） | [ ] |
| 7.3 | Code splitting — dashboard view lazy load | [ ] |
