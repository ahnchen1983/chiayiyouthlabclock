...
## [Unreleased] - 2026-07-07 — 排班月曆修正 + 外勤申請 + 登入預設打卡頁

### 新增 (Added)
- **外勤申請（公出／出差／遠端工作）**：`LeaveType` 新增 `公出` / `出差` / `遠端工作` 三類，沿用請假申請與審核流程，但定位為「非請假」：
  - 不佔假別餘額（前後端餘額檢查自然跳過，餘額頁不顯示配額列）。
  - 不扣薪、不計入薪資單請假時數／明細（`calculations.ts` 以 `isOutingLeaveType()` 排除）。
  - 不計入月結報表假別分布（既有 guard 自動忽略未知類型）。
  - 核准後該時段出勤對照沿用既有機制，不列為缺勤。
  - 表單改名「請假／外勤申請」，類別下拉以 optgroup 區分請假／外勤；審核頁改名「請假／外勤審核」。
- 新增單元測試：外勤核准單不計請假時數、不扣薪（`tests/calculations.test.ts`，共 184 tests）。

### 變更 (Changed)
- **排班管理移除「遠端工作／出差」班別選用**（客戶回饋：與實際不符，遠端/出差改走外勤申請）。`StaffRole` 的 `remoteWork` / `businessTrip` 型別保留，既有排班資料仍正常顯示；編輯舊資料時保留原角色選項不被隱性改掉；時間軸圖例只顯示現場角色。
- **管理端登入預設頁改為「我的打卡」**（原為總覽儀表板；員工端原本即為打卡頁）。導覽標籤同步更名：請假審核 → 請假/外勤審核、我的請假 → 請假/外勤申請。

### 修正 (Fixed)
- 排班管理月曆：修正「新增遠端/出差角色」重構時引入的重複顯示 bug。角色分組的初始物件 `EMPTY_ROLE_GROUPS` 為模組層級常數，淺拷貝（`{ ...EMPTY_ROLE_GROUPS }`）後五個角色的陣列仍是共用參照，導致每個日期格、每次重新 render 都往同一批陣列累加 shifts，畫面上排班內容越疊越多。改為 `emptyRoleGroups()` 工廠函式，每格取得全新陣列。（`components/admin/ScheduleManager.tsx`）

### 測試
- 驗證基準更新：184 個 Vitest、5 條 Playwright e2e 全數通過。
