      ...
      <div className="lg:col-span-2 space-y-8">
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500 mb-1">Weekly Hours</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-slate-500 mb-1">Next Shift</p>
              ...
              <div key={record.id} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Verified via Corporate WiFi</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-slate-800">{record.clockIn} - {record.clockOut || '--:--'}</p>
                  <p className="text-xs text-emerald-500 font-medium">Valid Entry</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-indigo-600 text-white p-6 rounded-2xl shadow-lg shadow-indigo-100">
          ...
          <div className="space-y-4">
            {upcomingShifts.map((shift) => (
              <div key={shift.id} className="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20">
                <div className="flex justify-between items-start mb-2">
                  <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${SHIFT_TYPES[shift.type].color.replace('bg-', 'bg-opacity-20 bg-')}`}>
                    {shift.type}
                  </span>
            ...
            {upcomingShifts.length === 0 && (
              <p className="text-sm text-indigo-100 italic">No shifts scheduled for this week.</p>
            )}
          </div>
        </div>

        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h3 className="text-slate-800 font-bold mb-3">Notice Board</h3>
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-100 text-sm text-amber-800">
            <strong>Maintenance:</strong> Corporate WiFi will be down for update this Sunday from 02:00 to 04:00 AM.
          </div>
        </div>
      </div>
    </div>
